...
add_binds("normal", {
    ...
    -- Play embedded video in youtube-viewer
    --[[
        key({}, "v", function (w)
            local view = w.view
    ...
    -- Play embedded video in mplayer
    key({}, "v", function (w)
        local view = w.view
        local uri = view.hovered_uri or view.uri
        if uri then
            luakit.spawn(string.format("urxvt -e mplayer %q", uri))
        end
    end),

    -- Download and play embedded video in mplayer
    key({}, "V", function (w)
        local view = w.view
        local uri = view.hovered_uri or view.uri
        if uri then
            luakit.spawn(string.format("urxvt -e cclive -s best --filename '%%t.%%s' "
                .. "--output-dir %q/luakit_video/ --exec='mplayer \"%%f\"' %q", os.getenv("HOME"), uri))
        end
    end),
})
